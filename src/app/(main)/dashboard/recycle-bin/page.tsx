...
export default function RecycleBinPage() {
  ...
  const loadDeletedLeads = async () => {
    try {
      ...
      // Fetch Lost leads (includes both Leads page and Renewals page items marked Lost)
      const leadsData = await fetchWithAuth("/api/crm/leads?stage=Lost");
      ...
      if (!notCalledStageId) {
        ...
        if (notCalled?.stage_id) setNotCalledStageId(notCalled.stage_id);
      }
